...
class AligNet(nn.Module):
    ...
    def forward(self, fixed, moving, return_neg = False, hyp_tensor=None):
        ...
        tensors    = get_same_dim_tensors([x5_1, y5_1, deconv5, upsamp6to5], y5_1.size(-1), -1)
        tensors    = get_same_dim_tensors(tensors, y5_1.size(-2), -2)
        ...
        tensors    = get_same_dim_tensors([x4_1, y4_1, deconv4, upsamp5to4], y4_1.size(-1), -1)
        tensors    = get_same_dim_tensors(tensors, y4_1.size(-2), -2)
        ...
        tensors    = get_same_dim_tensors([x3_1, y3_1, deconv3, upsamp4to3], y3_1.size(-1), -1)
        tensors    = get_same_dim_tensors(tensors, y3_1.size(-2), -2)
        concat3    = torch.cat(tensors, dim=1)  # 130 x 64 x 64 # 195 x 24 x 24 x 26
        ...
        tensors    = get_same_dim_tensors([x2, y2, deconv2, upsamp3to2], y2.size(-1), -1)
        tensors    = get_same_dim_tensors(tensors, y2.size(-2), -2)
        concat2    = torch.cat(tensors, dim=1)    # 66 x 128 x 128    # 99 x 48 x 48 x 52
        ...
        tensors    = get_same_dim_tensors([x1, y1, deconv1, upsamp2to1], y1.size(-1), -1)
        tensors    = get_same_dim_tensors(tensors, y1.size(-2), -2)
        concat1    = torch.cat(tensors, dim=1)    # 34 x 256 x 256    # 51 x 96 x 96 x 104
